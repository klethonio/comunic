...
if(function_exists('getUser')){
	if(getUser($_SESSION['adUser']['id'], 2)){
		$pageId = $_GET['pageid'];
		if($readArt = read('cuc_posts', 'WHERE id=?', array($pageId), 'i')){
...
<?php
		if(!empty($_GET['pageid']) && !empty($_GET['delid'])){
			$idImg = $_GET['delid'];
		...
		echo '<ul rel="'.$pageId.'" class="gblist">';
		if($readGallery = read('cuc_gallery', 'WHERE post_id=?', array($pageId), 'i')){
			$i = 0;
...
?>
                	<li<?php if($i%6==0){echo ' class="last"';}?>>
                        ...
                        <div class="action">
                            ...
                            <a href="?url=pages/gallery&pageid=<?=$art['id']?>&delid=<?=$gallery['id']?>" title="Exluir imagem <?=$i?>"><img src="ico/no.png" title="Exluir imagem <?=$i?>" alt="Exluir imagem <?=$i?>" /></a>
                        </div><!-- /action -->
                    </li>
<?php
			}
