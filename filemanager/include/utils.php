...
function check_files_extensions_on_path($path, $ext)
{
	if ( ! is_dir($path))
	{
		...
		if ( ! in_array(mb_strtolower($fileinfo['extension']), (is_string($ext) ? [] : $ext)))
		{
			unlink($path);
		}
	}
...
{
	foreach ((is_string($phar) ? [] : $phar) as $file)
	{
		if ($file->isFile())
		{
			if (in_array(mb_strtolower($file->getExtension()), (is_string($ext) ? [] : $ext)))
			{
